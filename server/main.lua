...
--  MopsScripts | QBox — Profile pro Fahrzeugtyp
-- ══════════════════════════════════════════════════════════════════
...
RegisterNetEvent('hm-handling:server:RequestOpen', function(vehicle)
    ...
    DB_GetProfiles(vehicle, function(profiles)
        TriggerClientEvent('hm-handling:client:OpenMenu', src, {
            ...
            translations = Config.NUI.Translations,
            labels       = Config.NUI.Labels,
            valueLevels  = Config.NUI.ValueLevels,
        })
        Log(('Player %d öffnet Editor — Fahrzeug: %s — %d Profile'):format(src, vehicle, #profiles))
    end)
end)
